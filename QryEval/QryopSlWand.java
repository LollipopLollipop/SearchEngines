/**
 *  This class implements the WAND operator for all retrieval models.
 *
  ...
  public QryResult evaluateHelper (RetrievalModel r) throws IOException {
    ...
    for(Integer intDocId : qDMap.keySet()){
    	...
    	for(int j=0; j<qrySize; j++){
    		...
    		double toMultiply=Math.pow((qDMap.get(intDocId)[j]),this.weights.get(j)/this.totalWeight);
  ...
  public double getDefaultScore (RetrievalModel r, long docid) throws IOException {
	  ...
	  if (r instanceof Indri)
	  {
	    	...
	    	for(int i=0; i<qrySize; i++){
				...
				toMultiply = Math.pow(toMultiply, this.weights.get(i)/this.totalWeight);
				score*=toMultiply;
